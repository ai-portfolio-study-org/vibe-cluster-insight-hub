...
- 한국도로공사\_고속도로 출입시설 위치정보(FILE)\_1달\_1개월\_202505.csv
...
- **사용 데이터:**
  ...
  - (노선명, 역명, 역 종류, 위도, 경도)
- **계산 방식:**
  1. 산업단지 ↔ 가장 가까운 철도역 거리 계산 (Haversine)
  2. 거리 기반 정규화 점수 산정 (0~100 범위)
  3. 역 종류별 보정 점수 가산:
     ```
     final_score = min(100, normalized_distance_score + bonus_weight)
     bonus_weight = 10 if high_speed_railway else 5 if freight/transfer else 0
     ```
  4. 최종 점수는 100점을 초과하지 않도록 제한

#### 계산 예시

- 거리 3.5km → 정규화 점수: 85점
- 고속철도역인 경우: 85 + 10 = 95점 (최종)
- 일반역인 경우: 85 + 0 = 85점 (최종)
...
- **보정 요소:**
  ...
  - 공항별 인프라 상대 점수 기반 정규화
- **사용 데이터:**
  - 한국공항공사\_공항 위치정보\_20240801.csv (공항명, 위도, 경도)
  - 한국공항공사\_공항 계류장 및 주차장 정보\_20240719.csv (계류장수, 여객터미널수, 화물터미널수)
  - 한국공항공사\_공항별 교통정보\_20240726.csv
- **계산 방식:**
  1. 산업단지 ↔ 공항 거리 계산 (Haversine)
  2. 거리 기반 정규화 점수 산정 (0~100 범위)
  3. 공항 인프라 상대 점수 계산:
     ```
     infrastructure_score = (계류장수 + 여객터미널수*2 + 화물터미널수*3) / max_infrastructure
     bonus_weight = infrastructure_score * 20  # 최대 +20점
     ```
  4. 최종 점수 계산:
     ```
     final_score = min(100, normalized_distance_score + bonus_weight)
...
- 거리 8.0km → 정규화 점수: 75점
- 인프라 상대 점수 0.8 → 보정: 75 + (0.8 × 20) = 91점 (최종)

### 4. 항만 접근성 (가중치 0.10)

- **지표:** 산업단지 ↔ 최근접 항만 거리 + 항만 기능
- **보정 요소:**
  - TEU 처리능력 (컨테이너 수용량)
  - 항만 건설 공사 건수 및 규모
...
- **계산 방식:**
  1. 산업단지 ↔ 가장 가까운 항만까지 거리 계산 (Haversine)
  2. 거리 기반 정규화 점수 산정 (0~100 범위)
  3. 항만 기능 점수 계산:
     ```
     TEU_score = (TEU / max_TEU) * 100
     project_score = (공사수 / max_공사수) * 100
     port_function_score = 0.5 * TEU_score + 0.5 * project_score
     ```
  4. 최종 항만 접근성 점수:
     ```
     final_port_score = 0.6 * port_function_score + 0.4 * distance_score
     ```

#### 계산 예시

- 거리 140.0km → 거리 점수: 30점
- TEU 점수: 85점, 공사 점수: 75점
- 항만 기능 점수: 0.5 × 85 + 0.5 × 75 = 80점
- 최종 점수: 0.6 × 80 + 0.4 × 30 = 60점
...
업종별 특성에 따라 교통 인프라의 중요도가 다르므로, 업종별 가중치를 적용하여 더 정확한 접근성 점수를 산정합니다.

#### 업종별 가중치 예시

| 업종          | 고속도로 | 철도 | 공항 | 항만 | 합계 |
| ------------- | -------- | ---- | ---- | ---- | ---- |
| 바이오/제약   | 70       | 80   | 90   | 60   | 300  |
| IT/소프트웨어 | 60       | 70   | 95   | 40   | 265  |
| 반도체/전자   | 85       | 70   | 85   | 75   | 315  |
| 자동차부품    | 90       | 85   | 60   | 70   | 305  |
| 기계/정밀     | 85       | 80   | 70   | 75   | 310  |
| 철강/금속     | 80       | 95   | 50   | 90   | 315  |
| 시멘트/건자재 | 95       | 90   | 40   | 85   | 310  |

#### 정규화 계산 예시 - 바이오/제약 업종

**원래 가중치:** 고속도로 70 / 철도 80 / 공항 90 / 항만 60

**정규화:**

- W\_고속도로 = 70 / (70+80+90+60) ≈ 0.233
- W\_철도 = 80 / 300 ≈ 0.267
- W\_공항 = 90 / 300 ≈ 0.300
- W\_항만 = 60 / 300 ≈ 0.200
...
즉, 업종별 교통 전략 점수 = 기존 정규화된 각 점수 × 업종 가중치로 재조합한 것입니다.
