...
const scenarios = [
  ...
  { id: 2, name: "음성 스마트팩토리 단지", status: "진행중", date: "2024-01-20" },
    ...
    <Layout>
      <div className="p-6 max-w-7xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">정책 시뮬레이터</h1>
            <p className="text-gray-600 mt-2">가상 산업단지 조성 시나리오 분석 및 정책 효과 예측</p>
          </div>
          ...
          <div className="lg:col-span-1 space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>새로운 산업단지 조성 시나리오를 구성하세요</CardDescription>
              </CardHeader>
                ...
                <div>
                  ...
                  <Select value={industryType} onValueChange={setIndustryType}>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="smart-factory">스마트팩토리</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                ...
                <div className="space-y-3">
                  {scenarios.map((scenario) => (
                    <div key={scenario.id} className="flex items-center justify-between p-3 border rounded-lg">
                      ...
                      <Badge variant={scenario.status === '완료' ? 'default' : scenario.status === '진행중' ? 'secondary' : 'outline'}>
                        {scenario.status}
                      </Badge>
                    </div>
                  ))}
                    ...
                    <CardContent>
                      <ResponsiveContainer width="100%" height={300}>
                        <LineChart data={simulationResults.employment}>
                          ...
                          <YAxis />
                          <Tooltip />
                          <Line type="monotone" dataKey="before" stroke="#94a3b8" strokeDasharray="5 5" name="기존 고용" />
                          <Line type="monotone" dataKey="after" stroke="#3b82f6" strokeWidth={2} name="시나리오 적용 후" />
                        </LineChart>
                      </ResponsiveContainer>
                      <div className="mt-4 grid grid-cols-2 gap-4">
                        <div className="p-4 bg-blue-50 rounded-lg">
                          <h4 className="font-semibold text-blue-900">직접 고용</h4>
                          <p className="text-2xl font-bold text-blue-600">{employmentScale[0].toLocaleString()}명</p>
                        </div>
                        <div className="p-4 bg-green-50 rounded-lg">
                          <h4 className="font-semibold text-green-900">간접 고용</h4>
                          <p className="text-2xl font-bold text-green-600">{Math.round(employmentScale[0] * 1.5).toLocaleString()}명</p>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                    ...
                    <CardContent>
                      <ResponsiveContainer width="100%" height={300}>
                        <AreaChart data={simulationResults.traffic}>
                          ...
                          <YAxis domain={[0, 100]} />
                          <Tooltip />
                          <Area type="monotone" dataKey="congestion" stackId="1" stroke="#94a3b8" fill="#94a3b8" fillOpacity={0.5} name="현재 혼잡도" />
                          <Area type="monotone" dataKey="newCongestion" stackId="2" stroke="#ef4444" fill="#ef4444" fillOpacity={0.5} name="예상 혼잡도" />
                        </AreaChart>
                      </ResponsiveContainer>
                      <div className="mt-4 p-4 bg-yellow-50 rounded-lg">
                        <h4 className="font-semibold text-yellow-900">교통 개선 권고사항</h4>
                    ...
                    <CardContent>
                          ...
                          <div key={index} className="p-4 border rounded-lg">
                            ...
                            <div className="flex items-center justify-between mt-2">
                              <div>
                                ...
                                <p className="text-lg font-bold">{item.current}{item.unit}</p>
                              </div>
                              <div className="text-right">
                                <p className="text-sm text-gray-500">예상</p>
                                <p className="text-lg font-bold text-green-600">{item.predicted}{item.unit}</p>
                              </div>
                            </div>
                            <div className="w-full bg-gray-200 rounded-full h-2 mt-2">
                              <div 
                                className="bg-green-500 h-2 rounded-full transition-all duration-500"
                                style={{ width: `${(item.predicted / Math.max(item.current, item.predicted)) * 100}%` }}
                              ></div>
                            </div>
                          </div>
                        ))}
                      ...
                      <div className="mt-6 p-4 bg-green-50 rounded-lg">
                        ...
                        <p className="text-sm text-green-800 mt-2">
                          해당 시나리오는 지역 경제에 <strong>긍정적 영향</strong>을 미칠 것으로 예상됩니다. 
                          특히 고용 창출과 지역 GDP 성장에 크게 기여할 전망입니다.
                        </p>
                      </div>
                    </CardContent>
                  </Card>
              ...
              <Card>
                <CardContent className="flex items-center justify-center h-96">
                  <div className="text-center">
                    ...
                    <h3 className="text-lg font-semibold text-gray-900 mb-2">시뮬레이션 준비</h3>
