...
const Network = () => {
  ...
  const getConnectionType = (type: string) => {
    ...
    return (
      types[type as keyof typeof types] || { color: "bg-gray-500", label: type }
    );
  };
  ...
  const getNodeConnections = (nodeId: string) => {
    return mockNetworkData.links.filter(
      ...
      <div className="p-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            산업단지 내 기업간 협업 가능성과 공급망 관계를 시각화합니다
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <CardHeader>
                <CardTitle className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-4 text-xs">
                    {Object.entries(getConnectionType(""))
                      .slice(0, 4)
                      .map(([key, _], index) => {
                        const types = [
                          "공급망",
                ...
                <div className="relative w-full h-full bg-gradient-to-br from-gray-50 to-blue-50 rounded-lg border">
                  ...
                  <svg className="w-full h-full">
                    ...
                    {mockNetworkData.links.map((link, index) => {
                      const sourceNode = mockNetworkData.nodes.find(
                        (n) => n.id === link.source
                      );
                      const targetNode = mockNetworkData.nodes.find(
                        (n) => n.id === link.target
                      );
          ...
          <div className="space-y-6">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {getNodeConnections(selectedNode.id).map(
                        (connection, index) => {
                          const partnerId =
                            connection.source === selectedNode.id
                              ? connection.target
                              : connection.source;
                          const partner = mockNetworkData.nodes.find(
                            (n) => n.id === partnerId
                          );
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-center space-x-2">
                                <div
                                  className={`w-2 h-2 rounded-full ${connectionType.color}`}
                                ></div>
                ...
                <CardContent>
                  <div className="space-y-3">
                    ...
                    <div className="text-center">
                      <Badge
                        className={getConnectionType(selectedEdge.type).color}
                      >
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-medium">
                        {selectedEdge.strength.toFixed(1)}/1.0
                      </span>
                    </div>
                  </div>
                </CardContent>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-3">
                  {Object.entries(getNodesByIndustry()).map(
                    ([industry, companies]) => (
                      <div
                        key={industry}
                        className="flex items-center justify-between p-2 bg-gray-50 rounded"
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-medium">
                    {mockNetworkData.nodes.length}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600">협업 관계</span>
                  <span className="font-medium">
                    {mockNetworkData.links.length}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-medium">
                    {(
                      (mockNetworkData.links.length * 2) /
                      mockNetworkData.nodes.length
                    ).toFixed(1)}
                  </span>
                </div>
